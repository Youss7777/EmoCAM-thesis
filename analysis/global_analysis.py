"""
Pipeline to perform global analysis
"""
import os
import torch
import PIL
from explanations.emonet import ExplanationsEmonet
from analysis.local_analysis import LocalAnalysis
import pandas as pd
...
class GlobalAnalysis:
    ...
    def __init__(self, config, nb_objects, confidence_tresh, device=torch.device('cpu')):
        self.device = device
        self.local_analysis = LocalAnalysis(device=device)
        self.expl_emo = ExplanationsEmonet(device=device, config=config)
        self.dataset_path = config.PATH_DATASET
        self.nb_objects = nb_objects
        self.confidence_tresh = confidence_tresh
    ...
    def update_emonet_df(self, file_path, df_emonet):
        ...
        max_emotion, max_prob, emotion_tensor, arousal, valence, vis, grayscales = self.expl_emo.get_explanations_for_image(file_path, image_name)
        ...
        return df_emonet, max_emotion, max_prob, grayscales

    def update_yolo_df(self, file_path, max_emotion, max_prob, grayscales, df_yolo):
        """
        ...
        new_df_yolo = self.local_analysis.local_analysis(file_path=file_path, max_emotion=max_emotion, max_prob=max_prob,
                                                         cam_output=grayscales, nb_objects=self.nb_objects,
                                                         confidence_thresh=self.confidence_tresh)
    ...
    def save_model_outputs(self):
        ...
        # Get image paths
        image_paths = self.get_image_paths(self.dataset_path)
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                df_emonet, max_emotion, max_prob, grayscales = self.update_emonet_df(image_path, df_emonet)

                # update yolo dataframe with outputs from new image
                df_yolo = self.update_yolo_df(image_path, max_emotion, max_prob, grayscales, df_yolo)
    ...
    def analyze(self):
        # get info
        total_number_folders = self.get_number_of_folders(self.dataset_path)
        ...
        nb_images_to_process = self.get_number_of_images(self.dataset_path, _nb_elems_to_process)
        print("Total number of images to process = ", nb_images_to_process)
        # Sanity check
        print(f"Total number of image paths: {len(self.get_image_paths(self.dataset_path))}")
        # save model outputs & get number of images actually processed
        nb_img_processed = self.save_model_outputs()
