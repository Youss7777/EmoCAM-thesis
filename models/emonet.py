...
from models.alexnet_big import AlexNetBigEmoNet
...
class EmoNet:
    ...
    def __init__(self, path_emonet, b_eval=True):
        self.path_emonet = path_emonet
        self.emonet = self.get_emonet(b_eval=b_eval)

    def get_emonet(self, b_eval=True):
        """
        ...
        emonet.load_state_dict(torch.load(self.path_emonet))
        if b_eval:
            emonet.eval()
...
class EmoNetPreProcess(object):
    ...
    """
    def __init__(self, path_img_mean):
        ...
        with open(path_img_mean, 'r') as fin:
            for line in fin:
                line = line.strip()
                parts = line.split(',')
                row = []
